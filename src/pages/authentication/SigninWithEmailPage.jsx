                ...
                <div className="row align-items-center justify-content-center">
                    <div className="col-md-5">
                        <form onSubmit={handleOnSubmit} className='border px-3 py-4 p-md-5'>
                            <h2 className='login_form_title'>Signin with Email</h2>
                            <div className="row">
                                ...
                                <div className="col-md-12">
                                    ...
                                    <div className="row gap-2 my-3">
                                        <div className='d-flex align-items-center gap-2'>
                                            <button type="submit" className={`login_btn ${authentication.isLoading ? "bg-secondary" : "bg-danger"}`} disabled={authentication.isLoading}><BiLogInCircle />{authentication.isLoading ? "Please Wait" : "Login Google"}</button>
                                            <button type="submit" className={`login_btn ${authentication.isLoading ? "bg-secondary" : "bg-success"}`} disabled={authentication.isLoading}><BiLogInCircle />{authentication.isLoading ? "Please Wait" : "Login Facebook"}</button>
                                        </div>
                                    </div>
                                    <p className='already_have_account'>I don't have an account - <Link to="/signup-with-email">Sign Up</Link></p>
                                    <p className='already_have_account'>Continue with - <Link to="/signin-with-phone">Phone</Link> or <Link to="/signin-with-google">Google</Link></p>
                                </div>
                            </div>
                        </form>
                    </div>
